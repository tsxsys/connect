        ...
        passwordValidation: function (_options, _callback, _confirmcallback) {
            ...
            function buildPasswordRegex() {
                ...
                if (_options.noUpper) cases.push({
                    ...
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password must contain at least <strong class="invalid">' + _options.minUpperCase + '</strong> Upper Case Letters.</span>'
                });
                if (_options.noLower) cases.push({
                    'regex': '(?=' + CHARSETS.lowerCaseSet + ')',
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password can\'t contain a Lower Case Letter</span>'
                });
                else cases.push({
                    'regex': '(?=' + ('[' + CHARSETS.lowerCaseSet + '][^' + CHARSETS.lowerCaseSet + ']*').repeat(_options.minLowerCase) + ')',
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password must contain at least <strong class="invalid">' + _options.minLowerCase + '</strong> Lower Case Letters.</span>'
                });
                if (_options.noDigit) cases.push({
                    'regex': '(?=' + CHARSETS.digitSet + ')',
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password can\'t contain a Number</span>'
                });
                else cases.push({
                    'regex': '(?=' + ('[' + CHARSETS.digitSet + '][^' + CHARSETS.digitSet + ']*').repeat(_options.minDigits) + ')',
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password must contain at least <strong class="invalid">' + _options.minDigits + '</strong> Digits.</span>'
                });
                if (_options.noSpecial) cases.push({
                    'regex': '(?=' + CHARSETS.specialSet + ')',
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password can\'t contain a Special Character</span>'
                });
                else cases.push({
                    'regex': '(?=' + ('[' + CHARSETS.specialSet + '][^' + CHARSETS.specialSet + ']*').repeat(_options.minSpecial) + ')',
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password must contain at least <strong class="invalid">' + _options.minSpecial + '</strong> Special Characters.</span>'
                });

                cases.push({
                    'regex': '[' + charsetToString() + ']{' + _options.minLength + ',}',
                    'message': '<span class="invalid fa fa-times"></span><span class="pass__info">Password must contain at least <strong class="invalid">' + _options.minLength + '</strong> characters.</span>'
                });
            ...
            function validateField() {
                ...
                if (_options.failRepeats && $(_element).val().search(new RegExp('(.)' + ('.*\\1').repeat(_options.maxRepeats - 1), 'gi')) != -1) {
                    failedCases.push('<span class="invalid fa fa-times"></span><span class="pass__info">Password can not contain <strong class="invalid">' + _options.maxRepeats + '</strong> of the same character case-insensitive.</span>');
                }
                if (_options.failConsecutive && $(_element).val().search(new RegExp('(?=(.)' + ('\\1').repeat(_options.maxConsecutive) + ')', 'g')) != -1) {
                    failedCases.push('<span class="invalid fa fa-times"></span><span class="pass__info">Password can\'t contain the same character more than <strong class="invalid">' + _options.maxConsecutive + '</strong> times in a row.</span>');
                }
