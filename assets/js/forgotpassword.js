    ...
    if ((email == "")) {
      ...
      $.ajax({
        ...
        if(json.status == 1){
            ...
            $("#message").fadeOut(0, function (){
                $(this).html('<div class="alert alert-success icons-alert"><p>'+json.response+'</p></div>').fadeIn();
            });
            $('#cl_recover').hide();

        } else {
            $("#message").fadeOut(0, function (){
                $(this).html('<div class="alert alert-danger icons-alert"><p>'+json.response+'</p></div>').fadeIn();
            });
        }
        },
        beforeSend: function () {
