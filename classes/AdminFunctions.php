<?php
/**
 * Connect\AdminFunctions extends AppConfig
 */
namespace Connect;
...
class AdminFunctions extends AppConfig
{
    ...
    */
    ...
    public function getAllDepts($request, $columns): array
    {
        $bindings = array();

        try {
            $sql = "SELECT d.`dept_id`, d.`dept_name`, d.`dept_head`, 
                        CONCAT_WS(' ', mi.`firstname`, mi.`lastname`) AS `full_name`,
                    CASE WHEN
                      d.required = 0 THEN concat('<button id=\'editDepartment_', d.dept_id, '\' onclick=\'editDepartment(',d.dept_id,')\' class=\'btn btn-outline-bright-warning btn-sm\'>Edit</button>')
                      ELSE null END
                    AS action
                    FROM ".$this->tbl_departments." d
                    lEFT JOIN ".$this->tbl_member_info." mi
                    ON d.dept_head = mi.userid
                    GROUP BY d.dept_id, d.dept_name, d.dept_head";
