...
var url = 'admin/inc/ajax/settings.handler.inc.php?csrf_token=' + $('meta[name=\'csrf_token\']').attr('value');

    /* HANDLES POPOVERS FOR USER INFO */
function userInfoPull(id, elem) {
    $.ajax({
        type: "POST",
        url: "admin/inc/ajax/user_getinfo.php",
...
function roleUsersList(id) {
    ...
    $.ajax({
        type: "POST",
        url: "admin/inc/ajax/role_getusers.php",
...
function openEditSoftwareModal(request_id) {
    $.ajax({
        type: "POST",
        url: "partials/pieces/table/settings-software-edit.piece.php",
        ...
        success: function (response) {
            ...
            //Switches
            var elem_jack_ = document.querySelector('.jack_');
            var init = new Switchery(elem_jack_),
                changeField = document.querySelector('.js-check-change-field'),
                elem_jack_info;
            if (elem_jack_.checked === true){
                elem_jack_info = 'Enabled';
            } else{
                elem_jack_info = 'Disabled';
            }changeField.innerHTML = elem_jack_info;
            elem_jack_.onchange = function() {
                if (elem_jack_.checked === true){
                    elem_jack_info = 'Enabled';
                } else{
                    elem_jack_info = 'Disabled';
...
function openAddSoftwareModal() {
    $.ajax({
        type: "POST",
        url: "partials/pieces/table/settings-software-add.piece.php",
        data: {"csrf_token": $('meta[name="csrf_token"]').attr("value")},
        success: function (response) {
            $("#action_modal").html(response);
            $('#x_add_software_99').modal('toggle');
            //Switches
            var elem_jack_ = document.querySelector('.jack_');
            var init = new Switchery(elem_jack_),
                changeField = document.querySelector('.js-check-change-field'),
...
$('#saveUsers').click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "admin/inc/ajax/role_updateusers.php",
        processData: false,
        contentType: false,
        data: sendData,
        beforeSend: function () {
...
$(document).ready(function () {
    var software_list_table = $('table#software_list');

    if (software_list_table.length) {
        software_list_table.DataTable({
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return '<div class="btn-group">\n' +
                            '<button type="button" class="btn btn-sm" onclick="openEditSoftwareModal(\'' + row[0] + '\')"><i class="fas fa-pencil-alt"></i></button>\n'+
                            '</div>';
                    }
                }
            ],
    ...
    function deleteRole(id, btn_id) {
        ...
        $.ajax({
            type: "POST",
            url: "admin/inc/ajax/roles_delete.php",
    ...
    $("#newRoleForm").submit(function (event) {
        ...
        $.ajax({
            url: "admin/inc/ajax/roles_add.php",
            type: "POST",
            data: {
                "roleName": roleName,
    ...
    $("#editRoleForm").submit(function (event) {
        ...
        $.ajax({
            url: "admin/inc/ajax/role_update.php",
            type: "POST",
            data: {
                "roleId": roleId,
                "roleName": roleName,
    ...
    function editRole(id) {
        ...
        $.ajax({
            type: "POST",
            url: "admin/inc/ajax/role_getdata.php",
            data: {"role_id": id, "csrf_token": $('meta[name="csrf_token"]').attr("value")},
            beforeSend: function () {
                $.LoadingOverlay('show', {
                    image: '../login/images/Spin-0.8s-200px.svg',
