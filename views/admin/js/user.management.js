        ...
        success: function (user_info) {
            ...
            for (var prop in user_info) {
                if (user_info[prop] != '' && user_info[prop] != null) {
                    if (prop == 'UserImage') {
                        user_info_html += '<br><div class="img-thumbnail"><img src="user/img/avatars/' + user_info[prop] + '" height="240px"></div>';
                    } else {
                        user_info_html += '<div><b>' + prop.replace(/([A-Z])/g, ' $1') + ': </b>' + user_info[prop] + '</div>';
                    }
                }
            }
            $(elem).attr('data-bs-content', user_info_html).popover('show', {"html": true});
        },
...
function userRolesList(id) {
    ...
    $.ajax({
        type: "POST",
        url: "admin/inc/ajax/user_getroles.php",
        data: {"user_id": id, "csrf_token": $('meta[name="csrf_token"]').attr("value")},
        async: false,
        beforeSend: function () {
            $.LoadingOverlay('show', {
...
$('#saveRoles').click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "admin/inc/ajax/user_updateroles.php",
        processData: false,
        contentType: false,
        data: sendData,
        beforeSend: function () {
...
function banUser(id, btn_id, ban_hours, ban_reason) {
    ...
    if (ban_hours !== 0) {
        $.ajax({
            type: "POST",
            url: "admin/inc/ajax/users_ban.php",
            data: {
                "uid": uidJSON, "ban_hours": ban_hours, "ban_reason": ban_reason,
...
$(document).ready(function () {
    usertable = $('#manage_user_list').DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                searchable: true,
                render: function (data, type, row) {
                    // var crypt_uid = CryptoJS.AES.encrypt(row[0], salt);
                    var crypt_uid = row[0];
                    return "<a role='button' class='badge badge-primary' data-bs-container=\"body\" data-bs-toggle=\"popover\" data-bs-placement=\"top\" data-bs-html='true' data-bs-trigger='focus' id='info_" + row[0] + "'>" + data + "</a>";
            ...
            },
            {
                ...
                searchable: false,
                render: function (data, type, row) {
                    return '<button class="btn btn-outline-bright-warning btn-sm pull-right" type="button" role="button" id="banbtn_' + row[0] + '" onclick="banUser(&apos;' + row[0] + '&apos;, &apos;banbtn_' + row[0] + '&apos;)"><i class="icofont icofont-not-allowed"></i>Ban</button>';
                }
            }
        ],
        columnDefs: [{
        ...
        serverSide: true,
        ajax: {
            url: "admin/inc/ajax/users_getallactive.php?csrf_token=" + $('meta[name="csrf_token"]').attr("value"),
            error: function (xhr, error, thrown) {
...
function deleteUser(id, btn_id) {
    ...
    $.ajax({
        type: "POST",
        url: "admin/inc/ajax/users_delete.php",
