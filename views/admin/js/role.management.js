    ...
    $.ajax({
        ...
        success: function(user_array){
            ...
            $.each(user_array['role_users'], function(key, value){
                $('#users-selected').append("<option value='"+value.id+"'>"+value.full_name+" <span class='un_opt'> ["+value.username+"]</span></option>");
            });
            $.each(user_array['diff_users'], function(key, value){
                $('#users-available').append("<option value='"+value.id+"'>"+value.full_name+" <span class='un_opt'> ["+value.username+"]</span></option>");
            });
  ...
  roleTable = $('#role_list').DataTable({
    ...
    columns: [
        ...
        },
        {
          ...
          searchable: false,
          render: function(data, type, row){
            return "<button id='usersbtn_"+row[0]+"' class='btn btn-primary btn-round btn-sm m-1'>Assign Users</button>";
          }
        },
        {
          name: "edit role",
    ...
    paging: true,
    ajax: {
      url: "admin/inc/ajax/roles_getall.php?csrf_token="+ $('meta[name="csrf_token"]').attr("value"),
      error: function (xhr, error, thrown) {
