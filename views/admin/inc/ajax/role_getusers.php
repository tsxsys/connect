...
try {
    ...
    if ($request->valid_token() && ($auth->isSuperAdmin() || $auth->hasPermission('Assign Users to Roles'))) {
        ...
        foreach ($users as $uid => $user) {
            array_push($combo_array['all_users'], array( 'id'=>$user['id'], 'username'=>$user['username'], 'full_name'=>$user['full_name'] ));
            array_push($combo_array['diff_users'], array( 'id'=>$user['id'], 'username'=>$user['username'], 'full_name'=>$user['full_name'] ));
        }

        foreach ($role_users as $ruid => $role_user) {
            array_push($combo_array['role_users'], array( 'id'=>$role_user['id'], 'username'=>$role_user['username'], 'full_name'=>$role_user['full_name']));
        }
