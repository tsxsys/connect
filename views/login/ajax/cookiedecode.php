...
try {
    $cookie = Connect\CookieHandler::decodeCookie("usertoken");

    if ($cookie['status'] == true) {
        $cookiedata = JWT::decode($cookie["value"], $conf->jwt_secret, array('HS256'));

        $cookieDbPull = Connect\CookieHandler::validateCookie($cookiedata->userid, $cookiedata->cookieid, $cookiedata->tokenid, (int)$conf->cookie_expire_seconds);
        ...
        if (!isset($cookieDbPull['tokenid'])) {
            ...
            //Valid cookie
            $user = Connect\UserHandler::pullUserById($cookieDbPull['userid']);
