...
try {
    $secret = Connect\AppConfig::pullSetting("jwt_secret");
...
try {
    if ($validToken && ($decoded->pw_reset == "true")) {
        ...
        // Pulls user by ID
        $user = Connect\UserHandler::pullUserById($userid);

        // Checks password against validation rules
        $pwresp = Connect\PasswordHandler::validatePolicy($pw1, $pw2, (bool) $config['password_policy_enforce'], (int) $config['password_min_length']);
        ...
        if ($pwresp['status'] == true) {
            ...
            if ($response['status'] == true) {
                ...
                //Failure
                Connect\MiscFunctions::mySqlErrors($response['message']);
            }
        } else {
