    ...
    foreach ($barmenu as $btn => $url) {
        if (is_array($url)) {
            ...
            foreach ($url as $sub_btn => $sub_url) {
                echo "<li><a href=\"" . (Connect\MiscFunctions::isAbsUrl($sub_url) ? $sub_url : $this->base_url . '/' . $sub_url) . "\">$sub_btn</a></li>";
            }

            echo "</ul>";
        } else {
            echo "<li><a href=\"" . (Connect\MiscFunctions::isAbsUrl($url) ? $url : $this->base_url . '/' . $url) . "\">$btn</a></li>";
        }
    }
...
if ($auth->isLoggedIn()) {
    $usr = Connect\ProfileData::pullUserFields($_SESSION['uid'], array('firstname', 'lastname'));
